...
class UserInfo(object):
    def __init__(self, args):
        auth_url = '%s/v3/auth/tokens' % args.url
        ...
        r = requests.post(auth_url, json=auth_req_body, headers=headers)
        if r.status_code != 201:
            raise Exception('Error: Failed to get keystone token for getting ' +
                'user info. Status code: ' + str(r.status_code))
        token = r.headers['x-subject-token']

        domains_url = '%s/v3/domains' % args.url
        params = {'name': 'Default'}
        headers = {'X-Auth-Token': token}
        r = requests.get(domains_url, params=params, headers=headers)
        if r.status_code != 200:
            raise Exception('Error: Failed to get domain list. Status code: ' +
                str(r.status_code))
        domain_id = r.json()['domains'][0]['id']

        users_url = '%s/v3/users' % args.url
        params = {'domain_id': domain_id, 'name__startswith': 'load_user'}
        r = requests.get(users_url, params=params, headers=headers)
        if r.status_code != 200:
            raise Exception('Error: Failed to get user list. Status code: ' +
                str(r.status_code))
        users = r.json()['users']

        if len(users) == 0:
            raise Exception('Error: No test users found with names '
                'starting with load_user')

        self._user_infos = []
        for user in users:
            self._user_infos.append({'username': user['name'],
                'domain_id': user['domain_id']})

    def get_random_user_info(self):
        rand_index = random.randrange(0, len(self._user_infos) - 1)
        return self._user_infos[rand_index]


class TestTracker(object):
    def __init__(self, args):
        self._args = args
        self._user_info = UserInfo(args)
...
class Request(object):
    def __init__(self, agent, request_gatherer, args, user_info, on_complete=None):
        self._agent = agent
    ...
    def start(self):
        user_info = self._user_info.get_random_user_info()
                ...
                'identity': {
                    ...
                    'password': {
                        'user': {
                            ...
                            'password': self._args.test_users_password
                        }
                    }
                },
                'scope': {
        ...
        auth_req_body = json.dumps(auth_req_body)
...
def main():
    ...
    parser.add_argument('--test-users-password')
